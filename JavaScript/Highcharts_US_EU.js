function acc_highcharts() {
    ...
    let obj = array_US_EU[i];
	    for(let prop in obj){
		 ...
		 let array_US = newArray_US_EU.filter(n => n.inputState_US_EU !== 'EURO');
		 let array_EU = newArray_US_EU.filter(n => n.inputState_US_EU !== 'U.S. dollar');

	let size_US = array_US.map(function(array_US) {
	 	return array_US.amount_US_EU;
 	});
	let size_EU = array_EU.map(function(array_EU) {
		return array_EU.amount_US_EU;
	});
	let date_item_date_US_EU = newArray_US_EU.map(function(newArray_US_EU) {
	 	return newArray_US_EU.item_date_US_EU;
	});
	let name_US_EU = newArray_US_EU.map(function(newArray_US_EU) {
