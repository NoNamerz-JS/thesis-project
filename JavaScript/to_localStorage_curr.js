function checkParamsCur() {
    const it = document.getElementById("accumulation_item_date").value;
    const am = document.getElementById("accumulation_Amount").value;
...
function to_localStorage_acc() {
			let old_obj_US_EU = JSON.parse(localStorage.getItem('obj_US_EU')) || [];
      ...
      if (inputState_US_EU == 'EURO') {
        ...
        old_obj_US_EU.push(obj_0);
			...
			let json_US_EU = JSON.stringify(old_obj_US_EU);
			let storage_US_EU = localStorage.setItem('obj_US_EU', json_US_EU);
}
